...
for(let i=0; i<categories.length; i++) {
  ...
  selectedCategory.appendChild(theOption);
}

selectedCategory.addEventListener("change", () => {
    activities.forEach((activity) => {
        if(selectedCategory.value === activity.category) {
            createTable(tbody, activity);
        }
    })
});

 
function createTable(myTable, myActivity) {
    const row = myTable.insertRow(-1);
      
    const cell1 = row.insertCell(0);
    cell1.innerHTML = myActivity.id;
      
    const cell2 = row.insertCell(1);
    cell2.innerHTML = myActivity.name;
      
    const cell3 = row.insertCell(2);
    cell3.innerHTML = `$${myActivity.price}`;
}
